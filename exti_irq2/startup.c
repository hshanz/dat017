...
#define EXTI3_IRQ0_BPOS 1
#define EXTI3_IRQ1_BPOS (1<<1)
#define EXTI3_IRQ2_BPOS (1<<2)
...
{
    if( (*EXTI_PR & EXTI3_IRQ0_BPOS) != 0){
        count++;
        *EXTI_PR |= EXTI3_IRQ0_BPOS;
        
    }
    if((*EXTI_PR & EXTI3_IRQ1_BPOS) != 0){
        count = 0;
        *EXTI_PR |= EXTI3_IRQ1_BPOS;
    }
    if((*EXTI_PR & EXTI3_IRQ2_BPOS) != 0){
        *GPIO_ODR_HIGH = ~*GPIO_ODR_HIGH;
    }
...
void init_app()
{
    ...
    *((volatile unsigned int *) 0x40020C00) = 0x55555555; //Port-D alla utgångar (ska det vara så?)
   // *((volatile unsigned int *) 0x40021000) = 0x0; //port E 
     
    
    
    //Config sys
    *SYSCFG_EXTICR1 &= 0x0000; // Nollställ bitfältet 
    *SYSCFG_EXTICR1 |= 0x4444; //Ställ in 0100 för PE
    
    //config Exti
    *EXTI_IMR |= 0xF; //Öppna exti3 för avbrott
    *EXTI_RTSR |= 0xF; //positiv flank
    *EXTI_FTSR &= ~0xF;
